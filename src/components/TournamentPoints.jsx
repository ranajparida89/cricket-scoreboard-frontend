...
function supportsSubregionalFlags() {
  try {
    ...
    const w2 = ctx.measureText("ENG").width;
...
/* ------------------------------------------------------------------
   PREDICTION HELPERS (moved OUTSIDE the component to stop ESLint nagging)
-------------------------------------------------------------------*/
const modeLabel = (m) =>
  m === "S8" ? "Super 8" : m === "S6" ? "Super 6" : "Semi-finals";

const fmtNrr = (n) =>
  (safeNum(n, 0) >= 0 ? "+" : "") + safeNum(n, 0).toFixed(2);
...
function scoreAndProb(list) {
  ...
  const wrArr = list.map(
    (r) => safeNum(r.wins) / Math.max(1, safeNum(r.matches_played))
  );
  ...
  const scored = list.map((r) => {
    ...
    const score = 0.55 * ptsNorm + 0.35 * wrNorm + 0.1 * nrrNorm;
    return { ...r, score };
  });
...
function buildPredictions(list) {
  ...
  if (stage.mode === "S8")
    ...
    summary =
      "Exactly 9 teams detected: the format is Super 6 → Semi-finals (Top 3 + 1 wildcard by Points, then NRR tiebreak).";
  else
    summary =
      "Not enough teams for Super 8 or Super 6; going straight to Semi-finals.";
...
function cutoffNarrative(p) {
  ...
  const parts = [
    ...
    `${last.team_name} currently holds the last qualifying spot with ${safeNum(
      last.points
    )} pts and NRR ${fmtNrr(last.nrr)}.`,
  ];
  if (out) {
    parts.push(
      `${out.team_name} is next in line (${safeNum(out.points)} pts, NRR ${fmtNrr(
        out.nrr
      )}). To enter the Top ${p.cut}, they must finish with more points than ${
        last.team_name
...
export default function TournamentPoints({ isAdmin: isAdminProp }) {
  ...
  const loadFilters = useCallback(
    async (type) => {
      const actualType = type ?? matchType;
      try {
        const { data } = await axios.get(`${API_URL}/tournaments/filters`, {
          params: { match_type: actualType },
        });
        ...
        try {
          ...
          cats.forEach((c) =>
            (Array.isArray(c.editions) ? c.editions : []).forEach((e) => {
              if (!actualType || actualType === "All" || e.match_type === actualType) {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get(`${API_URL}/tournaments/leaderboard`, {
        params,
      });
  ...
  const polyPoints = (series, w, h, pad, yMin, yMax) => {
    ...
    return series
      .map((d, i) => `${pad + i * st},${yScale(d.value)}`)
      .join(" ");
  };
  ...
  const TD = ({ row, field, className = "", children }) => {
    ...
    const isNumberField =
      NUM_FIELDS.has(field) || FLOAT_FIELDS.has(field);
    const stepAttr = FLOAT_FIELDS.has(field) ? "0.01" : "1";
      ...
      >
        {isEditing ? (
          <input
            ...
            step={isNumberField ? stepAttr : undefined}
            value={editing.value}
            onChange={(e) =>
              setEditing((ed) => ({ ...ed, value: e.target.value }))
            }
            onBlur={commitEdit}
            onKeyDown={(e) => {
          ...
          {isAdmin && (
            <span
              style={{
                ...
                border: "1px solid rgba(88,230,217,.25)",
                padding: "3px 8px",
                borderRadius: 8,
              }}
            >
          ...
          {pointsSeries.length ? (
            <svg viewBox={`0 0 ${W} ${H}`} className="linechart">
              ...
              {pointsSeries.map((d, i) => {
                ...
                let nY =
                  yR + (rVal >= 0 ? LABEL.N_POS_DY : LABEL.N_NEG_DY);
                ...
                return (
                  <g key={`${d.team}-${i}`}>
                    ...
                    <text className="val goldv" x={pX} y={pY}>
                      {d.value}
                    </text>
                    <text className="val tealv" x={nX} y={nY}>
                      {Number(rVal).toFixed(2)}
                    </text>
                    <text
                      className={`xlabel ${
                        rotateLabels ? "small rot" : ""
                      }`}
                      x={x}
                      y={H - PAD + 16}
                      dy="10"
      ...
      <div className="card tp-predict">
        ...
        <div className="predict-grid">
            ...
            <div className="predict-title">
              ...
              {predictions.mode !== "S8" && (
                <span className="section-note">
                  Not applicable for this tournament
                </span>
              )}
            </div>
            {predictions.mode === "S8" ? (
              ...
              <div className="empty small">
                Super 8 appears only when the tournament has{" "}
                <strong>more than 9</strong> teams.
              </div>
            )}
          </div>
          ...
          <div className="predict-col">
            ...
            {predictions.mode === "S6" ? (
              ...
              <div className="empty small">
                Super 6 appears only when the tournament has{" "}
                <strong>exactly 9</strong> teams.
              </div>
            )}
          </div>
          ...
          <div className="predict-col">
            <div className="predict-title">
              ...
              {predictions.mode === "S6" && (
                <span className="section-note">
                  Top 3 from Super 6 + 1 wildcard (Points → NRR)
                </span>
              )}
              {predictions.mode === "S8" && (
                <span className="section-note">
                  Picked from the Super 8 contenders
                </span>
              )}
            </div>
            <ol className="predict-list">
              {predictions.semifinal.map((t, i) => (
                <li key={`sf-${t.team_name}`} className="predict-item">
                  ...
                  <span className="p-bar">
                    <span
                      className="p-fill"
                      style={{ width: `${t.probability ?? 90}%` }}
                    />
                  </span>
                  <span className="p-prob">
                    {(t.probability ?? 90)}%
                  </span>
                  {predictions.mode === "S6" && i === 3 && (
                    <span className="wild-tag">Wildcard</span>
                  )}
                </li>
              ))}
            </ol>
        ...
        <div className="snapshot-card">
          <div className="snapshot-title">
            Qualification cutoff snapshot (Top {predictions.cut})
          </div>
          {predictions.cut > 0 && predictions.cutoff && (
            <p className="snapshot-help">
              {cutoffNarrative(predictions)}
            </p>
          )}
          {predictions.cut > 0 && predictions.cutoff ? (
            <ul className="snapshot-list">
              {predictions.cutoff.above && (
                <li>
                  ...
                  {" • "}
                  {safeNum(predictions.cutoff.above.points)} pts, NRR{" "}
                  {safeNum(predictions.cutoff.above.nrr).toFixed(2)}{" "}
                  <span className="delta pos">
                    (
                    {safeNum(predictions.cutoff.above.points) -
                      safeNum(predictions.cutoff.cutoffTeam.points)}{" "}
                    pts vs cutoff)
                  </span>
                </li>
              )}
              <li>
                <span className="snap-label">
                  On cutoff (#{predictions.cut})
                </span>
                <strong>{predictions.cutoff.cutoffTeam.team_name}</strong>
                {" • "}
                {safeNum(predictions.cutoff.cutoffTeam.points)} pts, NRR{" "}
                {safeNum(predictions.cutoff.cutoffTeam.nrr).toFixed(2)}
              </li>
              {predictions.cutoff.bubble?.length ? (
                <>
                  <li className="snap-sub">
                  ...
                  {predictions.cutoff.bubble.map((b) => (
                    <li key={`bubble-${b.team_name}`} className="bubble">
                      ...
                      <span
                        className={`delta ${
                          b.ptsBehind > 0 ? "neg" : "pos"
                        }`}
                      >
                        ({b.ptsBehind > 0 ? `-${b.ptsBehind}` : "+0"} pts vs
                        cutoff)
                      </span>
                      , NRR {safeNum(b.nrr).toFixed(2)}{" "}
                      <span
                        className={`delta ${
                          b.nrrBehind > 0 ? "neg" : "pos"
                        }`}
                      >
                        (
                        {b.nrrBehind > 0
                          ? `-${Math.abs(b.nrrBehind).toFixed(2)}`
                          : "+0.00"}{" "}
                        NRR vs cutoff)
                      </span>
                    </li>
            ...
            <div className="empty small">
              Cutoff will appear once there are at least{" "}
              {predictions.cut || 4} teams.
            </div>
          )}
          <div className="tie-note">
            Tie-breaks in this view: Points → NRR → Wins → Team name.
          </div>
        </div>
        ...
        <div className="modal" onClick={() => setInfoOpen(false)}>
          ...
          >
            ...
            <div className="modal-body">
              <p>
                Server computes totals from <code>match_history</code>{" "}
                (ODI/T20) and returns a leaderboard. Predictions are
                recalculated on each reload using Points, Win-rate and NRR.
                Stages are chosen automatically from the team count with your
                rules.
              </p>
              {isAdmin && (
                <p>
                  <strong>Admin:</strong> Double-click any cell in Standings
                  to edit it locally. Click <em>Reset to DB</em> to discard
                  edits.
                </p>
              )}
            </div>
