// ✅ src/components/PlayerStats.js — perfectly aligned dark leaderboard tables
...
const PlayerStats = () => {
  ...
  /* ---------- helpers ---------- */
  const sr = (runs, balls) => (Number(balls) > 0 ? (Number(runs) / Number(balls)) * 100 : 0);
  ...
  const rows = useMemo(() => {
    ...
    return performances.filter((r) => {
      ...
      const typeOK = type === "ALL" || String(r.match_type).toUpperCase() === type;
  ...
  const combined = useMemo(() => {
    ...
    for (const perf of rows) {
      ...
      let found = acc.find((x) => `${x.player_name}|${x.team_name}|${x.match_type}` === key);
      if (!found) {
        found = {
    ...
    <div className="container mt-4 text-white">
      ...
      <div className="ps-card">
        <div className="ps-header">
          <h3 className="ps-title">📈 Player Performance Stats</h3>
          <p className="ps-sub">Dark leaderboard with animated top-3 and gold glow</p>
        </div>
        ...
        <div className="ps-grid" role="table" aria-label="Match-wise player stats">
          ...
          <div className="ps-body" role="rowgroup">
            ...
            {rows.map((r, idx) => {
              const notOut = String(r.dismissed_status || r.dismissed || "").toLowerCase().includes("not");
              const runsDisp = notOut ? `${r.run_scored}*` : r.run_scored;
              const srDisp = (r.strike_rate ? Number(r.strike_rate) : sr(r.run_scored, r.balls_faced)).toFixed(2);

              return (
                <div className="ps-row" role="row" key={`${r.player_name}-${idx}`}>
                  <div className="cell player">
                    <button className="player-link" onClick={() => handlePlayerClick(r.player_name)}>
                      {r.player_name}
                    </button>
                  </div>
              ...
              {combined.map((p, i) => {
                ...
                return (
                  <div className={`ps-row-sum ${top}`} role="row" key={`${p.player_name}-${i}`}>
                    ...
                    <div className="cell">
                      <button className="player-link" onClick={() => handlePlayerClick(p.player_name)}>
                        {p.player_name}
                      </button>
                    </div>
                    ...
                    <div className="cell num">
                      {filters.matchType !== "ALL" && filters.matchType !== "" ? p.match_count : p.total_matches}
                    </div>
